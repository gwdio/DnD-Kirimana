...
class Accessory(BaseGameObject):
    def __init__(
        self,
        name: str,
        slot: str,  # e.g., "ring", "amulet", "bracelet"
        rarity: Optional[str] = None,
        # Any stat this accessory modifies
    ...
    def pretty_rep_short(self) -> str:
        ...
        return f"{self.name} [{self.slot or 'Accessory'}] (Rarity: {self.rarity or 'Common'}) — {bonus_str}"

    def pretty_rep_long(self) -> str:
        lines = [f"{self.name} — {self.slot or 'Accessory'} (Rarity: {self.rarity or 'Common'})"]
        bonuses = {k: v for k, v in self.stats.to_json().items() if v not in (0, None)}
        if bonuses:
